 ...
 */
package org.esa.beam.bop.meris.brr.auxdata;
...
public class AuxFile {
    ...
    /**
     * Reads an unsigned byte (as <code>int</code>) value for the given variable from this database file.
     *
     * @param varId the variable ID
     * @return the value
     * @throws IOException if an I/O error occurs
     */
    public int readUByte(String varId) throws IOException {
        final ProductData data = readRecord(varId, 0, 1, ProductData.TYPE_UINT8, null);
     ...
     * @param memoryData  an instance of data to be reused, must not be <code>null</code>
     * @throws IOException if an I/O error occurs
     */
    public void readRecord(String varId, int recordIndex, ProductData memoryData) throws IOException {
        readRecord(varId, recordIndex, memoryData.getNumElems(), memoryData.getType(), memoryData);
    }
    ...
    private long getOffset(final AuxDatasetInfo datasetInfo, final AuxVariableInfo variableInfo, int recordIndex) {
        ...
        final long offset = datasetOffset + variableOffset + recordIndex * datasetInfo.getRecordSize();
        return offset;
    }
