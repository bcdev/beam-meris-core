...
public class CloudClassification implements Constants {
    ...
    public void classify_cloud(DpmPixel pixel) {
        ...
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_LOW_NN_P, low_P_nn);
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_PCD_NN_P, true);    /* DPM #2.1.5-25 */
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_LOW_POL_P, low_P_poly);
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_PCD_POL_P, pcd_poly); /* DPM #2.1.12-12 */
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_CONFIDENCE_P, delta_p);
        ...
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_BRIGHT, bright_f);
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_SLOPE_1, slope_1_f);
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_SLOPE_2, slope_2_f);

        // table-driven classification- step 2.1.8
        // DPM #2.1.8-1
        boolean land_f = AlbedoUtils.isFlagSet(pixel.l2flags, F_LAND);
        ...
        pixel.l2flags = AlbedoUtils.setFlag(pixel.l2flags, F_CLOUD, is_cloud);
    }
    ...
    private void spec_slopes(DpmPixel pixel, boolean[] result_flags) {
        ...
        if (AlbedoUtils.isFlagSet(flags, F_LAND)) {   /* land pixel */
        ...
        bright_f = (lh.rhoAg[auxData.band_bright_n] >= rhorc_442_thr)
                || AlbedoUtils.isFlagSet(saturated, auxData.band_bright_n); /* DPM #2.1.7-10 */
        ...
        if (lh.rhoAg[auxData.band_slope_d_1] <= 0.0) {
            ...
            slope1_f = ((slope1 >= auxData.slope_1_low_thr) && (slope1 <= auxData.slope_1_high_thr))
                    || AlbedoUtils.isFlagSet(saturated, auxData.band_slope_n_1);
        }
        ...
        if (lh.rhoAg[auxData.band_slope_d_2] <= 0.0) {
            ...
            slope2_f = ((slope2 >= auxData.slope_2_low_thr) && (slope2 <= auxData.slope_2_high_thr))
                    || AlbedoUtils.isFlagSet(saturated, auxData.band_slope_n_2);
        }
    ...
    private boolean is_cloudy(boolean land_f, boolean bright_f,
        ...
        index = AlbedoUtils.setFlag(index, CC_BRIGHT, bright_f);
        index = AlbedoUtils.setFlag(index, CC_LOW_P_NN, low_P_nn);
        index = AlbedoUtils.setFlag(index, CC_LOW_P_PO, low_P_poly);
        index = AlbedoUtils.setFlag(index, CC_DELTA_P, delta_p);
        index = AlbedoUtils.setFlag(index, CC_PCD_NN, pcd_nn);
        index = AlbedoUtils.setFlag(index, CC_PCD_PO, pcd_poly);
        index = AlbedoUtils.setFlag(index, CC_SLOPE_1, slope_1_f);
        index = AlbedoUtils.setFlag(index, CC_SLOPE_2, slope_2_f);
        index &= 0xff;
